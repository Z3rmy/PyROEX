...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        atm_label1 = QLabel("CLO SNR LIM (V/V):")
        ...
        atm_label2 = QLabel("OPE SNR LIM (V/V):")
        ...
        self.tabs.addTab(self.tab1, "Observation Monitoring")
        self.tabs.addTab(self.tab2, "Observation Combination")
        self.tabs.addTab(self.tab3, "Data Integrity")
        self.tabs.addTab(self.tab4, "File Cutting")
    ...
    def update_tab1(self):
        ...
        if self.A_or_I == 'A' and current_text == 'Atm':
            ...
            try:
                ...
                # 将选项卡内容添加到选项卡小部件
                    if i==0:
                        self.tab_widget.addTab(tab_content, f"Original Observation")
                    else:
                        self.tab_widget.addTab(tab_content, f"Rate of Change")
    ...
    def PLOT_tab1_Atm(self,tab_name, checkboxes):
        ...
        if tab_name == '选项卡1':
            ...
            for label in ylabel:
                if label.startswith('S'):
                    y_units.add('SNR ROC (V/V/s)')
                elif label.startswith('L'):
                    y_units.add('Carrier Phase ROC (Cycle/S)')
                elif label.startswith('C'):
                    y_units.add('Pseudorange ROC (m/S)')
                elif label.startswith('O'):
                    y_units.add('Mode Phase ROC (Cycle/S)')
    ...
    def init_tab5(self):
        ...
        <p><b>Observation Monitoring:</b> Visualize raw ROEX observations and their changing rates.</p>
        <p><b>Observation Combination:</b> Analyze observation combinations (MW, GF, IF, DIF).</p>
        <p><b>Data Integrity:</b> Check data integrities.</p>
        <p><b>File Cutting:</b> Extract specific time ranges from ROEX files.</p>
        <p><b>Configuration:</b></p>
        <ul>
            <li>For Atm files: Set SNR limits and window sizes for CLOse-loop and OPEn-loop phase</li>
